...
TEST(WorldTest, Creation) {
    World<float> world(1, 2, 3, 0.1, 10, 4, 90, 100, 10);
    ...
    ASSERT_FLOAT_EQ(0.1, world.GetBorderWeight());
...
TEST(WorldTest, SetBehaviours) {
    ...
    ASSERT_FLOAT_EQ(0.01, world.GetBorderWeight());
}
...
TEST(WorldTest, MoveBoid0) {
    World<float> world(1, 1, 1, 0.1, 100, 1, 90, 100, 0);
...
TEST(WorldTest, MoveBoid1) {
    World<float> world(1, 1, 1, 0.1, 100, 1, 90, 100, 10);
    world.moveBoids();
    ASSERT_EQ(10, world.GetNumberOfBoids());
}
